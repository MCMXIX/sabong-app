<template>
    <div class="lg:max-w-[30%] mx-auto lg:h-[calc(100vh-96px)]">
        <div class=" order-1 flex flex-col lg:order-1 fight--info__container mt-4 mb-4 lg:mt-6 ">
            ...
            <p :class="[getStatusTextColor(oFightInfo.status), 'font-bold text-center text-3xl lg:text-4xl my-2 lg:mt-5 order-2 lg:order-2']">{{ getFightStatus(oFightInfo.status) }}</p>
        </div>
        <p v-if="(bSideErrorMessage === true)" class="block text-red-high text-sm text-center lg:mt-11">Choose a Side</p>
        <div class="betting__container--radio">
            <label>
                <input type="radio" value="M" v-model="oBetDetails.side" @click="bSideErrorMessage = false" class="hidden" name="sidePicker">
                <span class="side--checkmark--radio meron--radio">MERON</span>
            </label>
            <label>
                <input type="radio" value="W" v-model="oBetDetails.side" @click="bSideErrorMessage = false" class="hidden" name="sidePicker">
        ...
        <div class="relative h-max mt-4 top-0">
            ...
            min="100" @keyup="getInputAmountValue();" v-model.number="inputBetAmount">
            <p v-if="(bErrorAmount === true)" class="block text-red-high text-sm text-center">Amount has to be greater than or equal to 100</p>
            <button 
        ...
        <div class="betting__container--amount mt-8">
            <div class="bet__button--row">
                <input type="button" class="button--amount" value="100" @click="setBetAmount(100)" :disabled="bDisableButton"/>
                <input type="button" class="button--amount" value="200" @click="setBetAmount(200)" :disabled="bDisableButton"/>
                <input type="button" class="button--amount" value="300" @click="setBetAmount(300)" :disabled="bDisableButton"/>
            </div>
            <div class="bet__button--row">
                <input type="button" class="button--amount" value="500" @click="setBetAmount(500)" :disabled="bDisableButton"/>
                <input type="button" class="button--amount" value="1000" @click="setBetAmount(1000)" :disabled="bDisableButton"/>
                <input type="button" class="button--amount" value="2000" @click="setBetAmount(2000)" :disabled="bDisableButton"/>
            </div>
        </div>
        <div class="fixed z-[60] inset-0 overflow-y-auto" :class="{'hidden' : bShowModal}" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                        ...
                        <div class="flex flex-col">
                            <div class="bet--results text-center gap-4">
                                <div class="fight--number__container">
                                    <h3>FIGHT</h3>
                                    <h3>#{{ oFightInfo.fight_no }}</h3>
                                </div>
                                <div class="bet--amount__container">
                                    <h3 class="bet__label">AMOUNT: </h3>
                                    <h3 class="uppercase font-bold">{{ getFormattedTotalBet(oBetDetails.amount) }}</h3>
                                </div>
                                <div class="bet--side__container">
                                    <h3>SIDE: </h3>
                                    <h3 :class="[sSideTextColor, 'uppercase font-bold modal--side']">{{ sSelectedSide }}</h3>
                                </div>
                                <div class="bet--confirm__buttons mt-5">
                                    <button class="button--secondary py-2 px-4 mr-2" @click="toggleSubmitModal()">Cancel</button>
                                    <button class="button--primary py-2 px-4" @click="addBet()">Confirm</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
         ...
         */
        sSelectedSide() {
            return ((this.oBetDetails.side === 'W') ? 'WALA' : 'MERON');
        },
         ...
         */
        sSideTextColor() {
            return ((this.oBetDetails.side === 'M') ? 'text-green-med' : 'text-red-high');
        }
    },
    methods: {
        ...mapActions('oBet', ['addNewBet']),
         ...
         */
        checkSides: function() {
            if (!!this.oBetDetails.side === false) {
                this.bSideErrorMessage = true;
                return;
            }

            this.bSideErrorMessage = false;
            this.toggleSubmitModal();
        },

        /**
         * setBetAmount
         * @param int iBetAmount
         */
        setBetAmount: function(iBetAmount) {
            this.oBetDetails.amount = iBetAmount;
            this.checkSides();
        },
        ...
        checkInputAmount: function() {
            if (this.inputBetAmount < 100) {
                ...
                this.oBetDetails.amount = this.inputBetAmount;
                this.bErrorAmount = false;
                this.checkSides();
            }
        },
         ...
         */
        addBet : function() {
            if (this.oFightInfo.status === 'O') {
                this.oBetDetails.fight_no = this.oFightInfo.fight_no;
                this.addNewBet(this.oBetDetails);
                
                this.bShowModal = true;
                this.oBetDetails.status = '';
                this.oBetDetails.side = '';
                this.oBetDetails.amount = '';
                this.inputBetAmount = '';
                this.bDisableButton = false;

                //TODO: ADD GENERATION OF RECIEPT
                return;
            }

            alert('Please wait for the fight to be open.');
            this.bShowModal = true;
        }
    }
}
